...
class GANLoss(nn.Module):
    ...
    def __init__(self,
        ...
        #h = int(256/2**(depth-1))
        c = 1
        self.target_real = torch.tensor([1],device='cuda',dtype=torch.float).view(1,2).expand(batch_size,2)
        self.target_fake = torch.tensor([0],device='cuda',dtype=torch.float).view(1,2).expand(batch_size,2)
