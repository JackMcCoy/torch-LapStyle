...
class ThumbAdaConv(nn.Module):
    def __init__(self, style_contrastive_loss=False,content_contrastive_loss=False,batch_size=8, s_d = 64, size=256):
        ...
        self.layer_norm_in = nn.ModuleList([
            nn.Identity(),
            nn.Identity(),
            nn.LayerNorm((batch_size, 256, size//4, size//4)),
            nn.Identity(),
            nn.Identity(),
            nn.LayerNorm((batch_size, 128, size//2, size//2)),
            nn.Identity(),
            nn.LayerNorm((batch_size, 64, size, size)),
            nn.Identity(),
        ])
        self.layer_norm_out = nn.ModuleList([
            nn.LayerNorm((batch_size, 512, size//8, size//8)),
            nn.Identity(),
            nn.LayerNorm((batch_size, 256, size//4, size//4)),
            nn.Identity(),
            nn.Identity(),
