...
class DecoderAdaConv(nn.Module):
    def __init__(self):
        ...
        self.decoder_1 = nn.Sequential(
            ResBlock(512),
            ConvBlock(512, 256))
        self.kernel_2 = AdaConv(256, 2, s_d = self.s_d)
        self.decoder_2 = nn.Sequential(
            ResBlock(256),
            ConvBlock(256, 128)
        )
        self.kernel_3 = AdaConv(128, 4, s_d = self.s_d)
        self.decoder_3 = nn.Sequential(
            ConvBlock(128, 128),
            ConvBlock(128, 64)
        )
        self.kernel_4 = AdaConv(64, 8, s_d = self.s_d)
        self.decoder_4 = nn.Sequential(
            ConvBlock(64, 64),
            nn.ReflectionPad2d((1, 1, 1, 1)),
            nn.Conv2d(64, 3, kernel_size=3)
    ...
    def forward(self, sF, cF):
        ...
        x = self.decoder_2(x)
        x = self.upsample(x)
        adaconv_out['r2_1'] = self.kernel_3(style, cF['r2_1']).data
        x += adaconv_out['r2_1'].data
        x = self.decoder_3(x)
        x = self.upsample(x)
        adaconv_out['r1_1'] = self.kernel_4(style, cF['r1_1']).data
...
def calc_losses(stylized, ci, si, adaconv_out, cF, sF, encoder, decoder, disc_= None, calc_identity=True, mdog_losses = True, disc_loss=True):
    ...
    if calc_identity==True:
        ...
        l_identity4 = 0
        cb_loss = 0
    loss_c = content_loss(stylized_feats['r1_1'], adaconv_out['r1_1'], norm=True)
    for key in content_layers[1:]:
        loss_c += content_loss(stylized_feats[key], adaconv_out[key], norm=True).data
