...
def calc_GAN_loss_from_pred(prediction: torch.Tensor,
    ...
    h = 256
    if target_is_real:
        target_tensor = torch.ones(batch_size, c, h, h, dtype=torch.half, device=torch.device('cuda:0'))
    else:
        target_tensor = torch.zeros(batch_size, c, h, h, dtype=torch.half, device=torch.device('cuda:0'))
