...
class ThumbAdaConv(nn.Module):
    ...
    def forward(self, cF: torch.Tensor, sF, calc_style=True, style_norm= None):
        ...
        x = self.relu[0](checkpoint(self.attention_block[0], style_enc, cF['r4_1'], preserve_rng_state=False))
        ...
        x = self.relu[1](checkpoint(self.attention_block[2], style_enc, x, preserve_rng_state=False))
        ...
        x = self.relu[2](checkpoint(self.attention_block[5], style_enc, x,  preserve_rng_state=False))
        ...
        # in = 64 ch
        res = x
        x = self.relu[3](checkpoint(self.attention_block[7], style_enc, x,  preserve_rng_state=False))
        x = checkpoint(self.learnable[7], x, preserve_rng_state=False)
        x = res + x
        x = checkpoint(self.learnable[8], x, preserve_rng_state=False)
        x = self.relu[3](checkpoint(self.attention_block[8], style_enc, x, preserve_rng_state=False))
