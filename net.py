...
class DecoderVQGAN(nn.Module):
    def __init__(self):
        ...
        self.quantize_3 = VectorQuantize(32, 4800, transformer_size=2)
        self.quantize_2 = VectorQuantize(64, 6000, transformer_size=3)
        self.decoder_1 = nn.Sequential(
            ResBlock(512),
            ConvBlock(512,256))
    ...
    def forward(self, sF, cF):
        ...
        t = self.upsample(t)
        t += adain(cF['r3_1'], sF['r3_1'])
        t, embed_ind, bl = self.quantize_3(t)
        book_loss += bl
        t = self.decoder_2(t)
        t = self.upsample(t)
        t += adain(cF['r2_1'], sF['r2_1'])
        t, embed_ind, bl = self.quantize_2(t)
        book_loss += bl
        t = self.decoder_3(t)
        t = self.upsample(t)
        book_loss += bl
