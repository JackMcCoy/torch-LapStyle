        ...
        for i in torch.split(real.detach(), 256, dim=2):
            for j in torch.split(i.detach(), 256, dim=3):
                ...
                if self.relgan:
                    ...
                    loss_Ds.append((
                            torch.mean((pred_real - torch.mean(pred_fake) - 1) ** 2) +
                            torch.mean((pred_fake - torch.mean(pred_real) + 1) ** 2)).unsqueeze(0)
                    )

                else:
                    loss_D_real = self.ganloss(pred_real, self.true)
                    loss_Ds.append(((loss_D_real + loss_D_fake) * 0.5).unsqueeze(0))
                idx += 1
        loss_Ds, ind = torch.cat(loss_Ds, dim=0).max(dim=0)
        return loss_Ds
