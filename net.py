...
class StyleAttention_w_Context(nn.Module):
    def __init__(self, chan, chan_out=None, s_d = 64, batch_size=4, padding=0, stride=1, key_dim=64, value_dim=64, heads=8,
                 size=32, adaconv_norm=True, context_ada_norm=True, norm_queries=False):
        super().__init__()
        self.chan = chan
        chan_out = chan if chan_out is None else chan_out
        ...
        self.context_ada_norm = context_ada_norm
    ...
    def forward(self, style_enc, x, context):
        ...
        q, k = map(lambda x: x * (self.key_dim ** -0.25), (q, k))
        if self.context_ada_norm:
            context = context * torch.rsqrt(torch.mean(context ** 2, dim=1, keepdim=True) + 1e-8)
...
class ThumbAdaConv(nn.Module):
    def __init__(self, style_contrastive_loss=False,content_contrastive_loss=False,batch_size=8, s_d = 64, size=256):
        ...
        self.attention_block = nn.ModuleList([
            ...
            StyleAttention(256, s_d=s_d, batch_size=batch_size, heads=12, size=int(size / 2 ** 2),
            ...
            StyleAttention(128, s_d=s_d, batch_size=batch_size, heads=6, size=int(size / 2 ** 1),
                           kernel_size=self.kernel_size, adaconv_norm=False),

            nn.Identity(),
            #AdaConv(64, 8, s_d=self.s_d, batch_size=batch_size, norm=False, kernel_size = self.kernel_size),
            StyleAttention(64, s_d=s_d, batch_size=batch_size, heads=4, size=size,
                           kernel_size=self.kernel_size, adaconv_norm=False),
            AdaConv(64, 8, s_d=self.s_d, batch_size=batch_size, norm=False, kernel_size = self.kernel_size)
        ])
    ...
    def forward(self, cF: torch.Tensor, sF, calc_style=True, style_norm= None):
        ...
        x = checkpoint(self.layer_norm[1], x, preserve_rng_state=False)
        ...
        #####
        x = checkpoint(self.layer_norm[2], x, preserve_rng_state=False)
        ...
        ######
        # in = 64 ch
        x = checkpoint(self.layer_norm[3], x, preserve_rng_state=False)
        ...
        x = checkpoint(self.layer_norm[4], x, preserve_rng_state=False)
        x = x + checkpoint(self.attention_block[8], style_enc, x, preserve_rng_state=False)
        x = x + checkpoint(self.learnable[9], x, preserve_rng_state=False)
...
class Discriminator(nn.Module):
    def __init__(self, depth=5, num_channels=64, relgan=True, quantize = False, batch_size=5):
        ...
        self.body = nn.Sequential(*[copy.deepcopy(cell) for i in range(depth - 2)])
        self.tail = nn.Sequential(
            nn.Conv2d(num_channels, num_channels, kernel_size=1),
            nn.LeakyReLU(),
...
def loss_no_patch(stylized: torch.Tensor,
    ...
    for i in range(half):
        ...
        s_contrastive_loss = s_contrastive_loss + compute_contrastive_loss(reference_style, style_comparisons, 0.2,
                                                                           0)
