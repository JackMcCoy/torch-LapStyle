...
class RevisionNet(nn.Module):
    def __init__(self, input_nc=6):
        ...
        self.relu = nn.ReLU()
        self.reflect = nn.ReflectionPad2d((1, 1, 1, 1))
        UpBlock = []
    ...
    def forward(self, input, style):
        ...
        out = self.relu(self.reflect(self.adaconv_pre_res(out, style, norm=False)))
        out = self.resblock(out)
        out = self.relu(self.reflect(self.adaconv_post_res(out, style, norm=False)))
        res_block = out.clone()
...
class Style_Guided_Discriminator(nn.Module):
    def __init__(self, depth=5, num_channels=64, relgan=True):
        ...
        self.head = nn.Sequential(
            spectral_norm(nn.Conv2d(3,num_channels,3,stride=1,padding=1, padding_mode='reflect')),
            nn.LeakyReLU(0.2)
            )
