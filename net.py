...
class AdaConv_w_FF(nn.Module):
    def __init__(self, in_dims, out_dims, s_d, batch_size, norm=False, ng=1, kernel_size=3, size=16):
        ...
        self.ada = AdaConv(in_dims, ng, s_d=s_d, c_out=out_dims, kernel_size=kernel_size, norm=norm)
...
class AttentionAdaConv(nn.Module):
    def __init__(self, style_contrastive_loss=False,content_contrastive_loss=False,batch_size=8, s_d = 64, size=256):
        super(AttentionAdaConv, self).__init__()
        self.s_d = s_d
        self.kernel_size = 3
        self.ks = self.kernel_size if self.kernel_size>1 else 3
        depth = 2 if size>128 else 1
...
def loss_no_patch(stylized: torch.Tensor,
    ...
    for i in range(half):
        ...
        c_contrastive_loss = c_contrastive_loss + compute_contrastive_loss(reference_content, content_comparisons,
    '''
    return loss_c, loss_s, content_relt, style_remd, l_identity1, l_identity2, l_identity3, l_identity4, loss_Gp_GAN, 0, 0, mxdog_losses, 0, p_loss, cb_loss # gan, patch_gan,mxdog, contrastive
