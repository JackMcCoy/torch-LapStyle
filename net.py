...
def calc_GAN_loss(real: torch.Tensor, fake:torch.Tensor, crop_marks, disc_:torch.nn.Module):
    pred_fake = torch.clamp(disc_(fake),0,1)
    loss_D_fake = calc_GAN_loss_from_pred(pred_fake, False)
    pred_real = torch.clamp(disc_(real),0,1)
...
def calc_losses(stylized: torch.Tensor,
    ...
    if patch_loss:
        if patch_stylized is None:
            ...
            patch_disc_loss = 0
            patch_loss = 0
            for idx, (i,j) in enumerate(zip(patch_feats,patch_stylized)):
                ...
                if idx>0:
                    top_patch_feats = encoder(top_level_patch[idx])['r4_1']
                    patch_loss = patch_loss +content_loss(patch_feats, top_patch_feats, norm=False)
                else:
                    patch_loss = patch_loss*2
    else:
        patch_loss = 0
