    ...
    def __init__(self, style_contrastive_loss=False,content_contrastive_loss=False,batch_size=8, s_d = 64, size=256):
        ...
        self.residual = nn.ModuleList([
            ...
            nn.Sequential(
                ...
                BlurPool(256, filt_size=3, stride=1, pad_type='replicate'),
                nn.LeakyReLU(),
            ...
            nn.Sequential(
                ...
                nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True),
                BlurPool(128, filt_size=3, stride=1, pad_type='replicate'),
                nn.LeakyReLU(),
                nn.Upsample(scale_factor=.5, mode='bilinear', align_corners=True),
            ),
            nn.Identity(),
            nn.Sequential(
                ...
                nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True),
                BlurPool(64, filt_size=3, stride=1, pad_type='replicate'),
                nn.LeakyReLU(),
                nn.Upsample(scale_factor=.5, mode='bilinear', align_corners=True)
            ),
            nn.Identity(),
