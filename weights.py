...
def downblock_weights():
    params = []
    for i,j,k,l in [(6, 128, False, 0), (128, 128, False, 0), (128, 64, False, 0), (64, 64, False, 128), (64, 64, False, 128)]:
        params.append(fused_conv_noise_weights(i, j, noise=k, noise_size=l))
    return nn.ModuleList(params)

def upblock_weights():
    params = []
    for i, j, k, l in [(64, 64, False, 256), (64, 128, False, 256), (128, 128, False, 256), (128, 3, False, 0)]:
        params.append(fused_conv_noise_weights(i, j, noise=k, noise_size=l))
