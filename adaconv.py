...
class KernelPredictor(nn.Module):
    def __init__(self, c_in, c_out, p, s_d):
        ...
        self.depthwise_kernel_conv = nn.Conv2d(s_d, self.c_out * (self.c_in//self.n_groups), 2, groups=self.n_groups)
        self.pointwise_avg_pool = nn.AvgPool2d(4)
        self.pw_cn_kn = nn.Conv2d(s_d, self.c_out*(self.c_out//self.n_groups), 1, groups=self.n_groups)
