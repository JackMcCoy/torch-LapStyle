...
class AdaConv(nn.Module):
    def __init__(self, c_in:int, p:int, batch_size:int = 8, s_d: int = 512, norm:bool=True, c_out=None, kernel_size=5):
        ...
        self.depthwise_kernel_conv = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Conv2d(self.c_in, self.c_out * (self.c_in//self.n_groups), kernel_size=3, padding=1 if kernel_size==5 else 0, padding_mode='reflect', groups=self.c_in))
        ...
        self.pw_cn_kn = nn.Sequential(
            nn.Conv2d(s_d, self.c_in, kernel_size=1),
            nn.LeakyReLU(),
            nn.Conv2d(self.c_in, self.c_out*(self.c_out//self.n_groups), kernel_size=1,groups=self.c_in))
        self.pw_cn_bias = nn.Sequential(
            nn.Conv2d(s_d, self.c_in, kernel_size=1),
            nn.LeakyReLU(),
            nn.Conv2d(self.c_in, self.c_out, kernel_size=1, groups=self.c_in))
        self.relu=nn.ReLU()
