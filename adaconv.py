...
class KernelPredictor(nn.Module):
    def __init__(self, c_in, c_out, p):
        ...
        self.depthwise_kernel_conv = nn.Sequential(
            nn.Conv2d(512, self.c_in*(self.c_in//self.n_groups), 2, groups = 512),
            nn.LeakyReLU())
        self.pointwise_avg_pool = nn.AvgPool2d(4)
        self.pw_cn_kn = nn.Sequential(nn.Conv2d(512, self.c_out*(self.c_out//self.pointwise_groups), 1),
                                      nn.LeakyReLU())
        self.pw_cn_bias = nn.Sequential(nn.Conv2d(512, c_out, 1),
                                        nn.LeakyReLU())
