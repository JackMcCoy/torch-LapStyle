...
def warmup_lr_adjust(optimizer, iteration_count, warmup_start=6.5e-8, warmup_iters=750, max_lr = 1e-3, decay=5e-5):
    """Imitating the original implementation"""
...
if args.train_model=='drafting':
    ...
    for i in tqdm(range(args.max_iter)):
        ...
        loss_c, loss_s, loss_r, loss_ss, l_identity1, l_identity2, l_identity3, l_identity4,mdog, loss_Gp_GAN, cb_i = losses
        loss = cb + loss_c * args.content_weight + loss_s * args.style_weight+\
                    loss_r * 10 + 18*loss_ss + l_identity1*50 + l_identity2 * 1 + l_identity3 * 50 + l_identity4 * 1 + mdog * .65 + loss_Gp_GAN * 5 + cb_i
        loss.backward()
        optimizer.step()
