...
def warmup_lr_adjust(optimizer, iteration_count, warmup_start=6.5e-8, warmup_iters=1500, max_lr = 1e-3, decay=5e-5):
    """Imitating the original implementation"""
...
if args.train_model=='drafting':
    ...
    for i in tqdm(range(args.max_iter)):
        ...
        loss_c, loss_s, loss_r, loss_ss, l_identity3, l_identity4,mdog, loss_Gp_GAN = losses
        loss = cb + loss_c * args.content_weight + loss_s * args.style_weight+\
                    loss_r * 12 + 18*loss_ss + l_identity3 * 50 + l_identity4 * 1 + mdog * 1 + loss_Gp_GAN * 5
        loss.backward()
        optimizer.step()
