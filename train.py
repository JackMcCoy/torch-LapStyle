...
def adaconv_thumb_train():
    ...
    for i in range(args.max_iter):
        ...
        with autocast(enabled=ac_enabled):
            ...
            loss_c, loss_s, content_relt, style_remd, l_identity1, l_identity2, l_identity3, l_identity4, mdog, loss_Gp_GAN, patch_loss, loss_moment = losses
            loss = loss_c * args.content_weight + args.style_weight * loss_s + content_relt * args.content_relt + style_remd * args.style_remd + patch_loss * args.patch_loss + mdog + l_identity1*50 + l_identity2 + l_identity3*50 + l_identity4 + loss_moment
        ...
        if (i + 1) % 1 == 0:
            ...
            for l, s in zip(
                    [loss, loss_c, loss_s, style_remd, content_relt, patch_loss,
                     mdog, loss_moment],
                    ['Loss', 'Content Loss', 'Style Loss', 'Style REMD', 'Content RELT',
                     'Patch Loss', 'MXDOG Loss','Moment Loss']):
                if type(l) == torch.Tensor:
                    loss_dict[s] = l.item()
            if(i +1) % 10 ==0:
