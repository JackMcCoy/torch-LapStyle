...
def drafting_train():
    ...
    wandb.watch(dec_, log='all', log_freq=250)
    scaler = GradScaler()
...
def adaconv_thumb_train():
    ...
    disc_ = torch.jit.trace(build_disc(
        disc_state, args.disc_depth) , torch.rand(args.batch_size, 3, 256, 256,device='cuda'), check_trace=False, strict=False)
    disc2_ = [torch.jit.trace(build_disc(state, args.disc2_depth), torch.rand(args.batch_size, 3, 256, 256,device='cuda'), check_trace=False, strict=False) for state in disc2_state]
    dec_optimizer = torch.optim.AdamW(dec_.parameters(recurse=True), lr=args.lr)
    ...
    for n in tqdm(range(args.max_iter), position=0):
        ...
        if n>2 and n % args.disc_update_steps == 0:
            ...
            for i in range(num_rev):
                ...
                res_in = F.interpolate(orig[:, :, crop_marks[i][0]:crop_marks[i][0]+128, crop_marks[i][1]:crop_marks[i][1]+128], 256, mode='bicubic')
        ...
        for i in range(num_rev):
            ...
            res_in = F.interpolate(
                orig[:, :, crop_marks[i][0]:crop_marks[i][0] + 128, crop_marks[i][1]:crop_marks[i][1] + 128], 256, mode='bicubic')
