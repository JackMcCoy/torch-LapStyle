...
if args.train_model=='drafting':
    ...
    with autocast(enabled=ac_enabled):
        ...
        disc_inputs = {'forward': (
        torch.rand(args.batch_size, 3, 256, 256).to(device), torch.rand(args.batch_size, 128, 4, 4).to(device)),
        'losses': (torch.rand(args.batch_size, 3, 512, 512).to(device), torch.rand(args.batch_size, 3, 256, 256).to(device), torch.rand(args.batch_size,128,4,4).to(device)),
