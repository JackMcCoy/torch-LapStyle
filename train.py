...
def warmup_lr_adjust(optimizer, iteration_count, warmup_start=1e-5, warmup_iters=250, max_lr = 1e-3, decay=5e-5):
    """Imitating the original implementation"""
...
if args.train_model=='drafting':
    ...
    for i in tqdm(range(args.max_iter)):
        ...
        with autocast(enabled=ac_enabled):
            optimizer.zero_grad()
            losses = calc_losses(stylized, ci.detach(), si.detach(), cF, sF, enc_, dec_, disc_, calc_identity=False, disc_loss=True, mdog_losses=True)
            ...
            loss = loss_c * args.content_weight + args.style_weight * (loss_s + 3*style_remd) +\
                        ...
                        l_identity3* 25 + l_identity4 * .5 + mdog * .65 + loss_Gp_GAN * 10 + cb_loss
