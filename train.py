...
def build_rev(depth, state):
    rev = net.ConvMixer(64, 16, kernel_size=5, patch_size=16).to(device)
    ...
    for n in tqdm(range(args.max_iter), position=0):
        ...
        with torch.no_grad():
            if (n + 1) % 50 == 0:
                ...
                draft_coco_grid = make_grid(invTrans(stylized), nrow=4, scale_each=True)
                ...
                save_image(draft_coco_grid, args.save_dir + '/drafting_mscoco_iter' + str(n + 1) + '.jpg')
