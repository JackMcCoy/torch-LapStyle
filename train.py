...
def revision_train():
    ...
    with autocast(enabled=ac_enabled):
        ...
        if args.load_rev == 1 or args.load_disc == 1:
            ...
            rev_state = None
        rev_ = torch.jit.trace(build_rev(args.revision_depth, rev_state),(torch.rand(args.batch_size,3,256,256,device='cuda:0'),torch.rand(args.batch_size,3,2048,2048,device='cuda:0'),torch.rand(args.batch_size,512*16,4,4,device='cuda:0'),torch.randint(256, (args.revision_depth, 2),device='cuda:0',dtype=torch.int32)), check_trace=False)
        #disc_inputs = {'forward': (
