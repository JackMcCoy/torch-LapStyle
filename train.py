...
def warmup_lr_adjust(optimizer, iteration_count, warmup_start=1e-6, warmup_iters=1000, max_lr = 1e-4, decay=5e-5):
    """Imitating the original implementation"""
...
if args.train_model=='drafting':
    ...
    for i in tqdm(range(args.max_iter)):
        ...
        with autocast(enabled=ac_enabled):
            optimizer.zero_grad()
            losses = calc_losses(stylized, ci.detach(), si.detach(), adaconv_out, cF, sF, enc_, dec_, disc_, calc_identity=False, disc_loss=True, mdog_losses=False)
            ...
            loss = loss_c * args.content_weight + args.style_weight * (loss_s + 3 * style_remd) +\
                        content_relt * 22 + l_identity1*50 + l_identity2 * 1 +\
        ...
        if (i + 1) % 10 == 0:
            ...
            print(f'c: {loss_c.item():.3f} s: {loss_s.item():.3f} \
              style_relt: {style_remd.item():.3f} content_relt: {content_relt.item():.3f}\
