    ...
    for n, (ci,si) in enumerate(zip(pl_content_iter,pl_style_iter)):
        ...
        with autocast(enabled=ac_enabled):
            with torch.no_grad():
                ...
                ci = [F.interpolate(ci, size=256, mode='nearest'), ci[:,:,:256,:256]]
                si = [F.interpolate(si, size=256, mode='nearest'), rc_si]
                cF = enc_(ci[0])
                sF = enc_(si[0])
            ...
            loss_D2 = calc_GAN_loss(si[-1], patch_stylized.clone().detach(), None, disc2_, device)
            loss_D = calc_GAN_loss(si[0], stylized.clone().detach(), None, disc_, device)
        ...
        with autocast(enabled=ac_enabled):
            ...
            p_losses = calc_losses(patch_stylized, ci[-1], si[-1], patch_cF, enc_, dec_, None, disc2_,
                                 ...
                                 sF=patch_sF, split_style=False, device=device)
...
if args.train_model == 'drafting':
    ...
    xmp.spawn(adaconv_thumb_train, args=(args,), nprocs=args.n_threads, start_method='fork')
