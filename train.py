...
if args.train_model=='drafting':
    ...
    for i in tqdm(range(args.max_iter)):
        ...
        with autocast(enabled=ac_enabled):
            '''
            optimizer.zero_grad()
            losses = calc_losses(stylized, ci.detach(), si.detach(), cF, enc_, dec_, calc_identity=False, disc_loss=False, mdog_losses=mdog_loss, remd_loss=remd_loss, sF=sF)
        ...
        with torch.no_grad():
            if (i + 1) % 50 == 0:
                ...
                out_images = make_grid([content_img_grid,style_source_grid,styled_img_grid], nrow=3)
    ...
    def build_disc(disc_state, disc_quant):
        disc=net.Discriminator(depth=args.disc_depth, num_channels=args.disc_channels, relgan=False).to(device)
    ...
    for i in tqdm(range(args.max_iter)):
        ...
        with autocast(enabled=ac_enabled):
            ci = next(content_iter).to(device)
            si = next(style_iter).to(device)
            ci = [F.interpolate(ci, size=256, mode='bicubic'), ci]
        ...
        with autocast(enabled=ac_enabled):
            ...
            losses = calc_losses(rev_stylized, ci_patch, si_cropped, cF, enc_, dec_, patch_feats, disc_, calc_identity=False, disc_loss=True, mdog_losses=False, content_all_layers=False, remd_loss=remd_loss)
            loss_c, loss_s, content_relt, style_remd, l_identity1, l_identity2, l_identity3, l_identity4, mdog, loss_Gp_GAN, patch_loss = losses
