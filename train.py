...
def build_rev(depth, state):
    rev = net.ConvMixer(128, 16, kernel_size=9, patch_size=16).to(device)
...
def adaconv_thumb_train():
    ...
    for n in tqdm(range(args.max_iter), position=0):
        ...
        mdog, loss_Gp_GAN, patch_loss, style_contrastive_loss, content_contrastive_loss, laplace_loss = losses
        ...
        loss = loss_patch_disc * args.gan_loss2 + loss_c * args.content_weight + \
               ...
               style_contrastive_loss * 0.3 + content_contrastive_loss * 0.3 + laplace_loss * args.style_weight
