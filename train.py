...
if args.train_model=='drafting':
    ...
    for i in tqdm(range(args.max_iter)):
        ...
        with autocast(enabled=ac_enabled):
            cF = enc_(ci_patch)
            sF = enc_(si_cropped)
            losses = calc_losses(rev_stylized, ci_patch, si_cropped, cF, enc_, dec_, patch_feats, disc_, gannoise, calc_identity=False, disc_loss=True, mdog_losses=False, content_all_layers=False, remd_loss=remd_loss, patch_loss=True, GANLoss=ganloss, sF=sF)
            loss_c, loss_s, content_relt, style_remd, l_identity1, l_identity2, l_identity3, l_identity4, mdog, loss_Gp_GAN, patch_loss = losses
        ...
        if (i + 1) % 10 == 0:
            loss_dict = {}
            for l, s in zip([loss, loss_c, loss_s, style_remd, content_relt, mdog, loss_Gp_GAN,loss_D, rev_stylized,patch_loss],
