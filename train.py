...
def adaconv_thumb_train():
    ...
    dec_optimizer = torch.optim.SGD(dec_.parameters(recurse=True), lr=args.lr, momentum=.9, decay=.9,eps=0.001)
    rev_optimizer = torch.optim.SGD(rev_.parameters(recurse=True), lr=args.lr, momentum=.9, decay=.9,eps=0.001)
    opt_D = torch.optim.SGD(disc_.parameters(recurse=True), lr=args.disc_lr, momentum=.9, decay=.9,eps=0.001)
    opt_D2 = torch.optim.SGD(disc2_.parameters(recurse=True), lr=args.disc_lr, momentum=.9, decay=.9,eps=0.001)
