...
def adaconv_thumb_train():
    with autocast(enabled=ac_enabled):
        ...
        rev_ = torch.jit.trace(build_rev(args.revision_depth, None),(torch.rand(args.batch_size,3,256,256,device='cuda:0'),torch.rand(args.batch_size,3,256,256,dtype=dtype,device='cuda:0')), check_trace=False, strict=False)
        random_crop = transforms.RandomCrop(256)
        if args.load_disc == 1:
            path = args.load_model.split('/')
            path_tokens = args.load_model.split('_')
