    ...
    def __init__(self, root, transform):
        ...
        if os.path.isdir(root):
            ...
            try:
                ...
                filelist = root.split('/')[-2]+'.txt'
                with open('/'.join(path[:-2])+'/'+filelist,'r') as file:
...
def drafting_train():
    ...
    if args.load_disc == 1:
        ...
        disc2_state = new_path_func('discriminator2_')
    else:
        disc_state = None
        disc2_state = None
        init_weights(dec_)
    dec_optimizer = torch.optim.AdamW(list(dec_.parameters(recurse=True)), weight_decay=args.weight_decay, lr=args.lr)
    '''    
    ...
    for n in tqdm(range(args.max_iter), position=0):
        ...
        loss_c, loss_s, content_relt, style_remd, l_identity1, l_identity2, l_identity3, l_identity4, loss_Gp_GAN_patch, loss_Gp_GAN, mdog, s_contrastive_loss, c_contrastive_loss, edge_loss = losses
        ...
        loss = loss_s * args.style_weight + loss_c * args.content_weight + content_relt * args.content_relt + \
               ...
               mdog * args.mdog_weight + edge_loss * .00005
...
def adaconv_128_train():
    ...
    for n in tqdm(range(args.max_iter), position=0):
        ...
        crop_marks = torch.randint(0, 127, (num_rev, 2))
        ci = [F.interpolate(ci, size=128, mode='bicubic').to(device)]+[F.interpolate(ci, size=256, mode='bicubic').to(device)] + [F.interpolate(crop_mark_extract(num_rev,crop_marks,ci,e), size=256, mode='bicubic').to(device) for e in range(num_rev)]
        si = [F.interpolate(si, size=128, mode='bicubic').to(device)]+[F.interpolate(e(si), size=256, mode='bicubic').to(device) for e in random_crop]
        cF = enc_(ci[0])
        sF = enc_(si[0])
        if n>2 and n % args.disc_update_steps == 0:
            dec_.eval()
            ...
            stylized_patches.append(patch_stylized)
            for i in range(num_rev):
                res_in = patch_stylized[:, :, crop_marks[i][0]:crop_marks[i][0]+128, crop_marks[i][1]:crop_marks[i][1]+128]
                patch_stylized = rev_[i](res_in.clone().detach().requires_grad_(True), ci[2+i])
                stylized_patches.append(patch_stylized)
...
def adaconv_thumb_train():
    ...
    for n in tqdm(range(args.max_iter), position=0):
        ...
        if n>2 and n % args.disc_update_steps == 0:
            ...
            for i in range(num_rev):
                orig = stylized if i==0 else patch_stylized
                res_in = orig[:, :, crop_marks[i][0]:crop_marks[i][0]+128, crop_marks[i][1]:crop_marks[i][1]+128]
...
if args.train_model == 'drafting':
    ...
    revlap_train()
elif args.train_model == 'adaconv_128':
    adaconv_128_train()
