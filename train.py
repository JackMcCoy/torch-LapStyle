...
def build_rev(depth, state):
    rev = net.ConvMixer(128, 16, kernel_size=7, patch_size=8, spe=True).to(device)
...
def adaconv_thumb_train():
    ...
    for n in tqdm(range(args.max_iter), position=0):
        ...
        patch_stylized, *_ = dec_(patch_cF['r4_1'].clone().detach().requires_grad_(True), style_emb.clone().detach().requires_grad_(True), calc_style=False, style_norm= style_norms)
        ...
        with torch.no_grad():
            res_in = F.interpolate(stylized[:,:,:128,:128], 256,mode='nearest')
        patch_cF = enc_(res_in)
        patch_stylized, *_ = dec_(patch_cF['r4_1'].clone().detach().requires_grad_(True), style_emb.clone().detach().requires_grad_(True), calc_style=False,
