...
class LapRev(nn.Module):
    def __init__(self, max_res, working_res, batch_size, s_d, momentumnet_beta):
        ...
        self.layers = revlib.ReversibleSequential(*[cell.copy(layer_num) for height, layer_num in self.num_layers],split_dim=0,coupling_forward=coupling_forward,coupling_inverse=coupling_inverse, memory_mode=revlib.core.MemoryModes.autograd_function, target_device='cuda')
    ...
    def forward(self, input:torch.Tensor, ci:torch.Tensor, style:torch.Tensor, enc_:nn.Module):
        ...
        out = self.layers(out.clone().detach().requires_grad_(), ci.data, style.data, enc_,layerwise_args_kwargs=None)

        out = input + torch.cat([out[:N,:,:,:256],out[N:,:,:,256:]],3)
        return out