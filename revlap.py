...
class LayerHolders(nn.Module):
    def __init__(self, max_res: int, working_res: int, layer_num: int, batch_size: int, s_d: int):
        ...
        self.module_patches = nn.ModuleList([Sequential_Worker(working_res//2, self.internal_layer_res, batch_size,s_d, i) for i in range(self.num_layers_per_side**2)])
...
class LapRev(nn.Module):
    def __init__(self, max_res, working_res, batch_size, s_d):
        ...
        self.layers = nn.ModuleList([LayerHolders(max_res, 512, i, batch_size, s_d) for i in range(num_layers)])
