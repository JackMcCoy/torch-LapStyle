...
class Sequential_Worker(nn.Module):
    ...
    def forward(self, x, ci, style):
        ...
        for ada, learnable in zip(self.adaconvs, self.upblock):
            ...
            out = learnable(out)
        print(out.shape)
        out = self.reinsert_work(x, out, row, col).repeat(1,2,1,1)
        print(out.shape)
        return out
...
class LayerHolders(nn.Module):
    def __init__(self, max_res: int, working_res: int, layer_num: int, batch_size: int, s_d: int):
        ...
        self.module_patches = sequential_to_momentum_net(nn.Sequential(*[Sequential_Worker(working_res//2, self.internal_layer_res, batch_size,s_d, i) for i in range(self.num_layers_per_side**2)]),residual=True,target_device='cuda')
