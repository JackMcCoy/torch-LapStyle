...
class RiemannNoise(nn.Module):
    ...
    def __init__(self, size:int, channels:int):
        ...
        self.params = nn.ParameterList([nn.Parameter(nn.init.normal_(torch.empty(size,size))).to(torch.device('cuda')),
            nn.Parameter(nn.init.normal_(torch.empty(size,size))).to(torch.device('cuda')),
            nn.Parameter(nn.init.constant_(torch.ones(1, ), .5)).to(torch.device('cuda')),
            nn.Parameter(nn.init.constant_(torch.ones(1, ), .5)).to(torch.device('cuda')),
            nn.Parameter(nn.init.constant_(torch.ones(channels,1,1), 0)).to(torch.device('cuda'))])
